...
// Get stylesheet
add_action('wp_enqueue_scripts', 'minimali_magazine_enqueue_styles');
function minimali_magazine_enqueue_styles()
{
    wp_enqueue_style('minimali-magazine-parent-style', get_template_directory_uri() . '/style.css');
}



// New fonts
function minimali_magazine_enqueue_assets()
{
    // Include the file.
    require_once get_theme_file_path('webfont-loader/wptt-webfont-loader.php');
    // Load the webfont.
    wp_enqueue_style(
        'minimali-magazine-fonts',
        wptt_get_webfont_url('https://fonts.googleapis.com/css2?family=Open+Sans:wght@400;500;600&family=Playfair+Display:wght@400;600&display=swap'),
        array(),
        '1.0'
    );
}
add_action('wp_enqueue_scripts', 'minimali_magazine_enqueue_assets');



function minimali_magazine()
{
    add_theme_support('custom-header', apply_filters('minimali_magazine_theme_custom_header_args', array(
        'default-image'          => '',
        ...
        'flex-width'            => true,
        'default-image'         => '',
        'wp-head-callback'       => 'minimali_magazine_theme_header_style',
    ) ) );
    register_default_headers( array(
        'header-bg' => array(
            'url'           => get_theme_file_uri( '/img/bg-img-min.png' ),
            'thumbnail_url' => get_theme_file_uri( '/img/bg-img-min.png' ),
            'description'   => _x( 'Default', 'Default header image', 'minimali-magazine' )
        ),  
    ) );
}
add_action('after_setup_theme', 'minimali_magazine', 9999999);

if (!function_exists('minimali_magazine_theme_header_style')) :
    /**
     * Styles the header image and text displayed on the blog.
     *
     * @see minimali_magazine().
     */
    function minimali_magazine_theme_header_style()
    {
